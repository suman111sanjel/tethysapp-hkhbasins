...
def get_returnPeriod_data(comid,region):
    request_params = dict(comid=comid, cty = region)
...
def get_historic_data(comid,region):
    hdates = []
    hvalues= []
    request_params = dict(comid=comid, cty = region)
    request_headers = dict(Authorization=AuthorizationToken)
...
def get_flow_duration_curve(comid,region):
    request_params = dict(comid=comid, cty = region)
    request_headers = dict(Authorization=AuthorizationToken)
...
def getForecastData(comid,region):
    request_params = dict(comid=comid, cty = region)
    request_headers = dict(Authorization=AuthorizationToken)
    ...
    return_obj = {}
    # Get Historic data
    hdate, hval,datetime_start,datetime_end = get_historic_data(comid,region)

    #Get Return Period
    return_max, return_2, return_10, return_20, return_5, return_50 = get_returnPeriod_data(comid,region)

    # Get flow duration
    prob, sorted_daily_avg = get_flow_duration_curve(comid,region)
...
def get_forecastpercent(comid,region):
    ens = ''

    request_params = dict(cty=region, comid=comid)
    ...
    for a in rivpercorder['two']:
        rivdates.append(a[0])
        if a[1] > 100:
            rivperctwo.append(100.00)
        else:
            rivperctwo.append(a[1])

    for b in rivpercorder['five']:
        if b[1] > 100:
            rivpercfive.append(100.00)
        else:
            rivpercfive.append(b[1])

    for c in rivpercorder['ten']:
        if c[1] > 100:
            rivpercten.append(100.00)
        else:
            rivpercten.append(c[1])

    for d in rivpercorder['twenty']:
        if d[1] > 100:
            rivperctwenty.append(100.00)
        else:
            rivperctwenty.append(d[1])

    for e in rivpercorder['fifty']:
        if e[1] > 100:
            rivpercfifty.append(100.00)
        else:
            rivpercfifty.append(e[1])

    for f in rivpercorder['max']:
        if f[1] > 100:
            rivpercmax.append(100.00)
        else:
            rivpercmax.append(f[1])
